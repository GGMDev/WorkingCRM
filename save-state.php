...
if ( isset( $_POST['str_xml'] ) && isset( $_POST['web'] ) && !isset( $_POST['save'] ) )
{
	...
	{
		echo "Επιλέξτε τουλάχιστον μια σειρά";
		return;
	}
...
elseif ( isset( $_POST['del'] ))
{
	    if (count(GetUserGroups()) > 1)
	    {
	        ...
	        foreach ( $arr_reports as $rpt ) {
	            ...
	            {
					if($s=="report")
						echo "<p>"."Δεν έχετε τα δικαιώματα να διαγράψετε αυτή την αναφορά"."</p>";
					else
						echo "<p>"."Δεν έχετε τα δικαιώματα να διαγράψετε αυτό το διάγραμμα"."</p>";
                	exit();
	            }
        	}
	    }
...
function Check_Crosstable_Group($arr,$is_save)
{
	...
	if($arr["group_fields"][count($arr["group_fields"])-1]["cross_table"]=="true")
	{
		...
		{
			echo "Επιλέξτε τουλάχιστον μια ομάδα πεδίων";
			exit();
		}
		$count_x=0;
		$count_y=0;
		...
		{
			echo "Επιλέξτε τουλάχιστον έναν άξονα";
			exit();
		}
	}
}
function Check_Crosstable_Totals($arr,$is_crosstable)
{
	if($is_crosstable=="true" && !empty($arr["totals"]))
	{
		...
		echo "Ελέγξτε τις συναρτήσεις της ομάδας";
		exit();
	}
}
?>