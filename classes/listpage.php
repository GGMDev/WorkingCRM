...
class ListPage extends RunnerPage
{
	...
	function addCommonHtml()
	{
		...
		if($this->is508) {
			$this->body["begin"].= "<a href=\"#skipdata\" title=\""."Μεταφορά στα δεδομένα του πίνακα"."\" class=\"".$this->makeClassName("s508")."\">"."Μεταφορά στα δεδομένα του πίνακα"."</a>";
			$this->body["begin"].= "<a href=\"#skipmenu\" title=\""."Μεταφορά στο μενού"."\" class=\"".$this->makeClassName("s508")."\">"."Μεταφορά στο μενού"."</a>";
			$this->body["begin"].= "<a href=\"#skipsearch\" title=\""."Μεταφορά σε αναζήτηση"."\" class=\"".$this->makeClassName("s508")."\">"."Μεταφορά σε αναζήτηση"."</a>";
			$this->body["begin"].= "<a href=\"templates/helpshortcut.htm\" title=\""."Βοήθεια για τις συντομεύσεις"."\" class=\"".$this->makeClassName("s508")."\">"."Βοήθεια για τις συντομεύσεις"."</a>";
		}
		...
		foreach($this->selectedRecs as $keys)
		{
			...
			if(!$lockRecord && @$_REQUEST["a"] == "delete" && $retval)
			{
						...
						if( $this->tName == ADMIN_USERS )
				{
					...
					$sql = "delete from ". $grConnection->addTableWrappers( "dbo.callcenter_ugmembers" )
...
//	normal mode row count
	...
	protected function orderFieldLabelString( $fName, $desc, $showLabelOnly )
	{
		...
		return $fName." ". ($desc ? "Φθίνουσα σειρά" : "Αύξουσα σειρά");
	}
