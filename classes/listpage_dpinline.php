...
class ListPage_DPInline extends ListPage_Embed
{
	...
	protected function getHeaderControlsBlocks()
	{
		...
		if( $this->inlineEditAvailable() && $this->xt->getVar("editselected_link") )
		{
			...
			if( !$this->isBootstrap()) 
				$buttons.= '<a class="rnr-button" href="#" '.$editselectedlink_attrs.' '.$editselectedlink_span.'>'."Επεξεργασία".'</a> ';
			else
				$buttons.= '<a class="' . $bs_button_class . ' " disabled href="#" '.$editselectedlink_attrs.' '.$editselectedlink_span.'>'."Επεξεργασία".'</a> ';
		};	
		...
		if( $this->updateSelectedAvailable() && $this->xt->getVar("updateselected_link") && $this->isBootstrap() )
		{
			...
			if( $this->isPD() )
				$buttons.= '<a class="' . $bs_button_class . '" disabled '.$updateselectedlink_attrs.'>'."Ενημέρωση των επιλεγμένων".'</a> ';
			else	
				$buttons.= '<a class="' . $bs_button_class . '" disabled href="#" '.$updateselectedlink_attrs.'>'."Ενημέρωση των επιλεγμένων".'</a> ';		  		
		}
		...
		if( $this->xt->getVar("saveall_link") )
		{
			...
			if( !$this->isBootstrap()) 
				$buttons.= '<a class="rnr-button" href="#" '.$savealllink_attrs.' '.$savealllink_span.'>'."Αποθήκευση όλων".'</a> ';					  
			else
				$buttons.= '<a class="' . $bs_button_class . '" href="#" '.$savealllink_attrs.' '.$savealllink_span.'>'."Αποθήκευση όλων".'</a> ';					  
		}
		...
		if( $this->xt->getVar("cancelall_link") )
		{
			...
			if( !$this->isBootstrap()) 
				$buttons.= '<a class="rnr-button" href="#" '.$cancelalllink_attrs.' '.$cancelalllink_span.'>'."Ακύρωση".'</a> ';					  
			else
				$buttons.= '<a class="' . $bs_button_class . '" href="#" '.$cancelalllink_attrs.' '.$cancelalllink_span.'>'."Ακύρωση".'</a> ';					  
		}
		...
		if( $this->deleteAvailable() && $this->xt->getVar("deleteselected_link") )
		{
			...
			// "bs-invisible-button" class need for init hidden in bootstrap
			if( !$this->isBootstrap()) 
				$buttons.= '<a class="rnr-button " href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'."Διαγραφή".'</a> ';		  
			else
				$buttons.= '<a class="' . $bs_button_class . '" disabled href="#" '.$deleteselectedlink_attrs.' '.$deleteselectedlink_span.'>'."Διαγραφή".'</a> ';		  
		}		
		if( $buttons ) 
		{
			if( !$this->isBootstrap()) 
