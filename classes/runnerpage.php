...
*/
	...
	function checkCaptcha()
	{
		...
		if ( $captchaSettings["type"] == FLASH_CAPTCHA && @strtolower($this->captchaValue) != strtolower(@$_SESSION["captcha_" . $this->getCaptchaId()]) )
		{
			...
			$this->message = "Ο κωδικός επιβεβαίωσης δεν είναι σωστός";
		}
	...
	function getCaptchaHtml($_captchaFieldName)
	{
		...
		$typeCodeMessage = "Πληκτρολόγησε τον κωδικό επιβεβαίωσης";
		$path = GetCaptchaPath();
...
*/		
	...
	function noRecordsMessage()
	{
		...
		if( $this->pSetSearch->noRecordsOnFirstPage() && !$isSearchRun )
			return "Δεν υπάρχει τίποτα για εμφάνιση. Εκτέλεσε πρώτα αναζήτηση.";
	...
	function buildPagination()
	{
		...
		else
		{
			...
			if($this->maxPages > 1)
			{
				...
				if($counterstart != 1)
				{
					$pagination.= $this->getPaginationLink(1,"Πρώτο") . $advSeparator;
					$pagination.= $this->getPaginationLink($counterstart - 1,"Προηγούμενο").$separator;
				}
				$pageLinks = "";
				...
				if($counterend != $this->maxPages)
				{
					$pagination.= $separator . $this->getPaginationLink($counterend + 1,"Επόμενο") . $advSeparator;
					$pagination.= $this->getPaginationLink($this->maxPages,"Τελευταίο");
				}
				if( $this->isBootstrap() )
	...
	function prepareRecordsIndicator($firstDisplayed, $lastDisplayed, $totalDisplayed)
	{
		...
		{
			$template = "Εμφανίζονται οι %first% - %last% από %total%";
	...
	*/
	...
	function getProceedLink()
	{
		...
		return '<span class="rnr-dbebrick">'
			...
			.  "Μεταφορά σε" . ' '. GetTableCaption( GoodFieldName( $this->tName ) )
			. '</a>'
			. "&nbsp;&nbsp;</span>";
	}
...
//		$this->AddJSFile('libs/js/migrationTool.js');
	...
	{
		if( $page == "add" )
			return GetTableCaption($table).", "."Εισαγωγή";
		if( $page == "edit" )
			return GetTableCaption($table).", "."Επεξεργασία"." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "view" )
			return GetTableCaption($table)." [". RunnerPage::getKeysTitleTemplate( $table, $pSet ). "]";
		if( $page == "export" )
			return "Εξαγωγή";
		if( $page == "import" )
			return GetTableCaption($table).", "."Εισαγωγή Αρχείου";
		if( $page == "search" )
			return GetTableCaption($table)." - "."Εύρεση";
		if( $page == "print" )
