...
class ExportPage extends RunnerPage
{
	...
	protected function getChooseFieldsCtrlMarkup()
	{
		...
		return '<select name="exportFields" multiple style="width: 100%;" data-placeholder="'."Παρακαλώ επιλέξτε".'" id="exportFields'. $this->id .'">'. implode( "", $options ) .'</select>';
	}
	...
	public function getValuesFromRow( &$row ) {
		...
		foreach( $this->selectedFields as $field )
		{
			...
			if( IsBinaryType( $fType ) )
				$values[ $field ] = "ΜΕΓΑΛΟΥ ΜΕΓΕΘΟΥΣ ΔΥΑΔΙΚΑ ΔΕΔΟΜΕΝΑ – ΔΕΝ ΕΙΝΑΙ ΔΥΝΑΤΗ Η ΑΠΕΙΚΟΝΙΣΗ";
			else
