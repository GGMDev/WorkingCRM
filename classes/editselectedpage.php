...
class EditSelectedPage extends EditPage
{
	...
	protected function prepareButtons()
	{
		...
		{
			$label = str_replace( "%n%", $this->nSelected, "Ενημέρωση %n% εγγραφών" );
	...
	{
		$message = str_replace( array("%succeed%", "%total%"), array( "<strong>".$this->nUpdated."</strong>", "<strong>".$this->nSelected."</strong>" ), "Ενημερώθηκαν με επιτυχία %succeed% από σύνολο %total% εγγραφές.");
		$this->setMessage( $message );
		
		if( $this->nUpdated != $this->nSelected ) {
			$message = str_replace( "%failed%", "<strong>".($this->nSelected - $this->nUpdated)."</strong>" , "Απέτυχαν %failed% εγγραφές.");
			$this->setMessage( $message );
		}
	}
