...
class AddPage extends RunnerPage
{
	...
	public static function handleBrokenRequest()
	{
		...
		if( postvalue("inline") )
		{
			...
			$returnJSON['message'] = "Πραγματοποίηση λάθους";
	...
	protected function recheckUserPermissions()
	{
		...
		{
			$this->setMessage( "Ο διαθέσιμος χρόνος σύνδεσης έχει εξαντληθεί."
				. "<a href='#' id='loginButtonContinue" . $this->id . "'>" . "Συνδεθείτε" . "</a>"
				. "για να αποθηκευτούν τα δεδομένα." );
		}
		else
		{
	...
	protected function setSuccessfulUpdateMessage()
	{
		...
		if( $this->mode == ADD_INLINE )
			$infoMessage = ""."Η εγγραφή σας καταχωρήθηκε"."";
		else
			$infoMessage = "<strong><<< "."Η εγγραφή σας καταχωρήθηκε"." >>></strong>";
		...
		else
		{
			...
			if( $this->editAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "edit", $keylink )."'>"."Επεξεργασία"."</a>&nbsp;";

			if( $this->viewAvailable() )
				$infoMessage.= "&nbsp;<a href='".GetTableLink( $this->pSet->getShortTableName(), "view", $keylink )."'>"."Προβολή"."</a>&nbsp;";
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != ADD_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."Η εγγραφή ΔΕΝ καταχωρήθηκε"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
		{
			$this->message = "Η εγγραφή ΔΕΝ καταχωρήθηκε".". ".$message;
		}
