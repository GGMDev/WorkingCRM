...
class RightsPage extends ListPage
{
	...
	function fillGroupsArr()
	{
		...
		$this->groups[-1] = "<"."Διαχειριστής".">";
		$this->groups[-2] = "<"."Εξ'όρισμού".">";
		$this->groups[-3] = "<"."Επισκέπτης".">";

		$sql = "select ".
			$grConnection->addFieldWrappers( "GroupID" ) .", ".
			$grConnection->addFieldWrappers( "Label" )
			." from ".
			$grConnection->addTableWrappers( "dbo.callcenter_uggroups" ) .
	...
	function getRights()
	{
		...
		$sql = "select ". $this->connection->addFieldWrappers( "GroupID" )
			...
			." from ". $this->connection->addTableWrappers( "dbo.callcenter_ugrights" )
	...
	function sortTables()
	{
		...
		if(count($this->alphaOrderedTables) > count($addedTables))
		{
			...
			$arr = array();
			$arr["collapsed"] = true;
			$arr["title"] = "Πίνακες εκτός λίστας";
			$arr["items"] = array();
			$this->menuOrderedTables[] = $arr;
	...
	function updateTablePermissions( $table, $group, $tableRights )
	{
		...
		$rightWTableName = $this->connection->addTableWrappers( "dbo.callcenter_ugrights" );
