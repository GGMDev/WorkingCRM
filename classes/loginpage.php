...
class LoginPage extends RunnerPage
{
	...
	protected function sendCodeBySMS()
	{
		...
		if( !$ret["success"] )
		{
			$this->message = "Σφάλμα κατά την αποστολή του μηνύματος"." ".$ret["error"];
		}
		else
		{
			$this->messageType = MESSAGE_INFO;
			$this->message = str_replace( "%phone%", $this->getMaskedPhone(), "Σας έχει σταλεί ένα μήνυμα κειμένου με τον κωδικό σας στο: %phone%" );
		}
	...
	protected function reportSecondAuthStepResult( $verified )
	{
		...
		if( !$verified )
			$returnJSON["message"] = "Λάθος κωδικός";
		else
		{
			if( $this->mode == LOGIN_POPUP )
				$returnJSON["message"] = "Έχετε συνδεθεί επιτυχώς.";
	...
	protected function refineMessage()
	{
		if( $this->message == "expired" )
			$this->message = "Ο διαθέσιμος χρόνος σύνδεσης έχει εξαντληθεί." . " " . "Παρακαλώ συνδεθείτε ξανά.";
		elseif( $this->message == "invalidlogin" )
			$this->message = "Ο χειριστής δεν υπάρχει";
	...
	protected function isLoginAccessAllowed()
	{
		...
		$this->message = mysprintf( "H πρόσβαση κλειδώθηκε για %s λεπτά", array($this->auditObj->LoginAccess()) );
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("dbo.CallCenter_users")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$sqlHead = "select * from ".$this->connection->addTableWrappers( "dbo.CallCenter_users" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "dbo.CallCenter_users", $data, $blobfields, $this );
			if( !$retval )
				return false;
