...
*/
	...
	public static function processEditPageSecurity( $table )
	{
		...
		if( $pageMode != EDIT_SIMPLE )
		{
			...
			if( !isLogged() || isLoggedAsGuest() )
				$messageLink = " <a href='#' id='loginButtonContinue'>". "Συνδεθείτε" . "</a>";
	...
	public static function handleBrokenRequest()
	{
		...
		else
		{
			if( postvalue('fly') )
			{
				...
				$_SESSION["message_edit"] = "<< "."Πραγματοποίηση λάθους"." >>";
		}
	}
	...
	protected function setSuccessfulEditMessage()
	{
		...
		$this->setMessage( "<strong>&lt;&lt;&lt; "."Η εγγραφή σας ανανεώθηκε". " &gt;&gt;&gt;</strong>" );
	}
	...
	protected function SecurityRedirect()
	{
		if($this->mode == EDIT_INLINE)
		{
			echo printJSON(array("success" => false, "message" => "Η εγγραφή δεν είναι επεξεργάσιμη"));
			exit();
		}
	...
	public function setDatabaseError( $message )
	{
		if( $this->mode != EDIT_INLINE )
		{
			$this->message = "<strong>&lt;&lt;&lt; "."Η εγγραφή ΔΕΝ επεξεργάστηκε"."</strong> &gt;&gt;&gt;<br><br>".$message;
		}
		else
			$this->message = "Η εγγραφή ΔΕΝ επεξεργάστηκε".". ".$message;
