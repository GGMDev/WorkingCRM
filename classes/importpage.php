...
class ImportPage extends RunnerPage
{
	...
	protected function prepareAndSentPreviewData()
	{
		...
		if( $returnJSON != false )
			...
			echo "Το αρχείο που προσπαθείς να εισάγεις δεν μπορεί να επεξεργαστεί";
	...
	protected function getBasicReportText( $totalRecords, $addedRecords, $updatedRecords, $isNotLogFile = true, $lineBreak = "<br>", $errorMessages = array(), $unprocessedData = array() )
	{
		...
		{
			$reportText .= "Εισαγωγή σε"." ".$this->strOriginalTableName.$lineBreak.
				str_format_datetime( db2time( now() ) ) .$lineBreak.$lineBreak;
		}
		
		$reportText .= mysprintf("%s από %s εγγραφές επεξεργάστηκαν σωστά.", array($boldBegin.$importedReords.$boldEnd, $boldBegin.$totalRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s εγγραφές προστέθηκαν.", array($boldBegin.$addedRecords.$boldEnd)) .$lineBreak.
			mysprintf("%s εγγραφές ενημερώθηκαν.", array($boldBegin.$updatedRecords.$boldEnd)) .$lineBreak;
			
		if( $notImportedRecords )
			$reportText.= mysprintf("%s εγγραφές με λάθος", array($boldBegin.$notImportedRecords.$boldEnd));
