...
class UploadHandler
{
    ...
    public function validate($uploadedFile, &$file, $error, $file_size, $index, $uploadDir) {
        ...
        if (!$file["name"]) {
            $file["error"] = "Το όνομα αρχείου δεν δόθηκε";
            return false;
        }
        if (!preg_match($this->options['accept_file_types'], $file["name"])) {
            $file["error"] = "Ο τύπος αρχείου δεν είναι αποδεκτός";
            return false;
        }
        if ($this->options['max_file_size'] && (
            ...
            $file["error"] = mysprintf("Το μέγεθος αρχείου είναι μεγαλύτερο από το όριο %s kbytes", array($this->options['max_file_size']));
            return false;
        }
        if ($this->options['min_file_size'] &&
            $file_size < $this->options['min_file_size'] * 1024) {
            $file["error"] = mysprintf("Το μέγεθος αρχείου πρέπει να είναι μεγαλύτερο %s kbytes", array($this->options['min_file_size']));
            return false;
        }
   		if (is_int($this->options['max_totalFile_size']) && (
                $this->getUploadFilesSize() + $file["size"] > $this->options['max_totalFile_size'] * 1024)
            ) {
            $file["error"] = mysprintf("Το σύνολο όλων των αρχείων είναι μεγαλύτερο από το όριο %s kbytes", array($this->options['max_totalFile_size']));
            return false;
        }
        if (is_int($this->options['max_number_of_files']) && (
            	...
            	if($this->options['max_number_of_files'] > 1)
            		$file["error"] = mysprintf("Μπορείτε να μεταφορτώσετε μέχρι %s αρχεία", array($this->options['max_number_of_files']));
            	else 
            		$file["error"] = "Μπορείτε να μεταφορτώσετε μόνο ένα αρχείο";
            return false;
        }
    ...
    public function handle_file_upload($uploadedFile, $index) {
		...
		if($this->pSet->isMakeDirectoryNeeded($this->field))
		{
			...
			{
				$file["error"] = "Ο κατάλογος μεταφόρτωσης αρχείων δεν υπάρχει";
				return $file;
			}
		}
