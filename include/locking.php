...
class oLocking
{
	var $lockTableName = "dbo.GGM_CallCenter_locking";
	var $ConfirmTime=250;
	var $UnlockTime=300;
	var $ConfirmAdmin;
	var $ConfirmUser;
	...
	function __construct()
	{
		...
		$this->ConfirmAdmin = "Ο διαχειριστής %s διέγραψε την περίοδο διόρθωσης εγγραφής";
		$this->ConfirmUser = "Η περίοδος διόρθωσης εγγραφής έληξε";
		$this->LockAdmin = "Ο χειριστής %s διορθώνει την εγγραφή για %s λεπτά";
		$this->LockUser = "Ένας άλλος χειριστής διορθώνει την εγγραφή";
		...
		if( $data = $qResult->fetchAssoc() )
		{
			...
			if($links){
				$str.='<a class="unlock" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'no\');return false;">'."Ακύρωση διόρθωσης".'</a>';
				$str.='<a class="edit" href="#" onclick="Runner.pages.PageManager.getAt(\''.runner_htmlspecialchars(jsreplace($strtable)).'\', '.$pageid.').locking.UnlockAdmin(\''
					.runner_htmlspecialchars(jsreplace($skeys)).'\',\''.$data["sessionid"].'\',\'yes\');return false;">'."Διόρθωση εγγραφής".'</a>';
			}
			return $str;
		}
