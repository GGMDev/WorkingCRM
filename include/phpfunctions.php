...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="dbo.CallsBackofficeView" && $field=="talkTime")
	{
		$value=gmdate("H:i:s", $data["talkTime"]);;
		return $value;
	}
				if($table=="dbo.CallsBackofficeView" && $field=="typingTime")
	{
		$value=gmdate("H:i:s", $data["typingTime"]);;
		return $value;
	}
				if($table=="dbo.CallsBackofficeView" && $field=="totalCallTime")
	{
		$value=gmdate("H:i:s", $data["totalCallTime"]);;
		return $value;
	}
				if($table=="dbo.CallsBackofficeView" && $field=="dialerCallEpoch")
	{
		$value = date('r', $value);
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="talkTime" && $ptype=="report")
	{
		$value=gmdate("H:i:s", $data["talkTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="talkTime" && $ptype=="print")
	{
		$value=gmdate("H:i:s", $data["talkTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="talkTime" && $ptype=="export")
	{
		$value=gmdate("H:i:s", $data["talkTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="talkTime" && $ptype=="view")
	{
		$value=gmdate("H:i:s", $data["talkTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="typingTime" && $ptype=="report")
	{
		$value=gmdate("H:i:s", $data["typingTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="typingTime" && $ptype=="print")
	{
		$value=gmdate("H:i:s", $data["typingTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="typingTime" && $ptype=="export")
	{
		$value=gmdate("H:i:s", $data["typingTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="typingTime" && $ptype=="view")
	{
		$value=gmdate("H:i:s", $data["typingTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="totalCallTime" && $ptype=="report")
	{
		$value=gmdate("H:i:s", $data["totalCallTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="totalCallTime" && $ptype=="print")
	{
		$value=gmdate("H:i:s", $data["totalCallTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="totalCallTime" && $ptype=="export")
	{
		$value=gmdate("H:i:s", $data["totalCallTime"]);;
		return $value;
	}
				if($table=="callsNegativeReport" && $field=="totalCallTime" && $ptype=="view")
	{
		gmdate("H:i:s", $data["totalCallTime"]);;
		return $value;
	}
				if($table=="vAgentPerformanceReport v2" && $field=="talkTime")
	{
		$value=gmdate('H:i:s', $data["talkTime"]);
		return $value;
	}
				if($table=="vAgentPerformanceReport v2" && $field=="typingTime")
	{
		$value=gmdate('H:i:s', $data["typingTime"]);
		return $value;
	}
				if($table=="vAgentPerformanceReport v2" && $field=="totalCallTime")
	{
		$value=gmdate('H:i:s', $data["totalCallTime"]);
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="dbo.Calls" && $field=="callDate" && $ptype=="edit")
	{
		return now();
	}
				if($table=="dbo.Calls" && $field=="callDate" && $ptype=="add")
	{
		return now();
	}
				if($table=="dbo.Calls" && $field=="callGdprAcceptance")
	{
		return Όχι;
	}
				if($table=="dbo.Calls" && $field=="callGdprRecordingAcceptance")
	{
		return Όχι;
	}
				if($table=="dbo.Calls" && $field=="crmUser")
	{
		return $_SESSION["UserID"];
	}
				if($table=="dbo.Items" && $field=="itemDiscount")
	{
		return 0;
	}
				if($table=="dbo.contacts" && $field=="dateInserted")
	{
		return now();
	}
				if($table=="dbo.contacts" && $field=="noCall")
	{
		return 0;
	}
				if($table=="dbo.phoneTypes" && $field=="dialerDnc")
	{
		return 0;
	}
				if($table=="dbo.internalComments" && $field=="entryUser")
	{
		return $_SESSION["UserName"];
	}
				if($table=="dbo.internalComments" && $field=="entryDate")
	{
		return now();
	}
				if($table=="dbo.novaApplications" && $field=="applicationAgentStatus")
	{
		return 1;
	}
				if($table=="dbo.manualCalls" && $field=="timeStamp")
	{
		return now();;
	}
				if($table=="callsForRedial" && $field=="callDate" && $ptype=="edit")
	{
		return now();
	}
				if($table=="callsForRedial" && $field=="callDate" && $ptype=="add")
	{
		return now();
	}
				if($table=="upcomingCallsForRedial" && $field=="callDate" && $ptype=="edit")
	{
		return now();
	}
				if($table=="upcomingCallsForRedial" && $field=="callDate" && $ptype=="add")
	{
		return now();
	}
				if($table=="dbo.voltonAgentApplications" && $field=="applicationAgentStatus")
	{
		return 1;
	}
				if($table=="dbo.CallsBackofficeView" && $field=="callDate" && $ptype=="edit")
	{
		return now();
	}
				if($table=="dbo.CallsBackofficeView" && $field=="callDate" && $ptype=="add")
	{
		return now();
	}
				if($table=="dbo.CallsBackofficeView" && $field=="callGdprAcceptance")
	{
		return Όχι;
	}
				if($table=="dbo.CallsBackofficeView" && $field=="callGdprRecordingAcceptance")
	{
		return Όχι;
	}
				if($table=="dbo.voltonApplicationsBackofficeView" && $field=="applicationAgentStatus")
	{
		return 1;
	}
				if($table=="dbo.novaApplicationsBackofficeView" && $field=="applicationAgentStatus")
	{
		return 1;
	}
				if($table=="dbo.novaAgentApplications" && $field=="applicationAgentStatus")
	{
		return 1;
	}
				if($table=="callsAgentView" && $field=="callDate" && $ptype=="edit")
	{
		return now();
	}
				if($table=="callsAgentView" && $field=="callDate" && $ptype=="add")
	{
		return now();
	}
				if($table=="callsAgentView" && $field=="callGdprAcceptance")
	{
		return Όχι;
	}
				if($table=="callsAgentView" && $field=="callGdprRecordingAcceptance")
	{
		return Όχι;
	}
				if($table=="callsAgentView" && $field=="crmUser")
	{
		return $_SESSION["UserID"];
	}
	return "";
...
function GetAutoUpdateValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="dbo.voltonApplications" && $field=="applicationLastStatusDate")
	{
		return now();
	}
				if($table=="dbo.novaApplications" && $field=="applicationLastStatusDate")
	{
		return now();
	}
				if($table=="dbo.manualCalls" && $field=="timeStamp")
	{
		return now();;
				...
				if($table=="dbo.voltonAgentApplications" && $field=="applicationLastStatusDate")
	{
		return now();
	}
				if($table=="dbo.voltonApplicationsBackofficeView" && $field=="applicationLastStatusDate")
	{
		return now();
	}
				if($table=="dbo.novaApplicationsBackofficeView" && $field=="applicationLastStatusDate")
	{
		return now();
	}
	return "";
