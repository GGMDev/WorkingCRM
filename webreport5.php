...
if (@$_SESSION['webreports']['settings']['title'] != "") {
	...
	$xt->assign("report_title",", "."Τίτλος".": ".$title);
} else {
...
if (@$_SESSION['webreports']['tables'][0] != "") {
	...
	$xt->assign("report_table",", "."Πίνακας".": ".$stable);
} else {
			...
			$.ajax({
				...
				success: function(msg){
					if ( msg == "OK" ) {
						...
						{
							$("#alert")
								.html("<p>'."Η Αναφορά αποθηκεύτηκε".'</p>")
				...
				error: function() {
					$("#alert").html("<p>'."Δημιουργήθηκαν προβλήματα κατά τη διαδικασία αποθήκευσης".'</p>").dialog("open");
				}
			});
		}
		thisid=this.id;
...
if(count($_SESSION['webreports']['group_fields'])>1)
{
	...
	<tr>
		<td class="tdimg rl" style="text-align:center">
			<img src="'.GetRootPathForResources("images/layout_stepped.png").'" id="stepped" border="0" alt="'."Κλιμακωτή Διάταξη".'" />
			<img src="'.GetRootPathForResources("images/layout_block.png").'" id="block" border="0" alt="'."Διάταξη Μπλοκ".'" />
		</td>
	</tr>
	<tr>
		<td class="tdimg rl" style="text-align:center">
		  <img src="'.GetRootPathForResources("images/layout_align.png").'" id="align" border="0" alt="'."Στοίχιση Διάταξης".'" />
...
else
{
	...
	<tr>
		<td class="tdimg rl" style="text-align:center">
		<img src="'.GetRootPathForResources("images/layout_tabular.png").'" id="stepped" border="0" alt="'."Διάταξη Πίνακα".'" />
		</td>
	</tr>
		';
