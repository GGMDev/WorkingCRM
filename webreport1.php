...
$(document).ready(function(){
	...
	$("#addrel_btn").click(function(){
		...
		if ( tbl_left == -1 ) {
			$("#alert").html("<p>'."Επιλέξτε τον αριστερό πίνακα".'</p>").dialog("open");
			return;
		}
		if ( tbl_right == -1 ) {
			$("#alert").html("<p>'."Επιλέξτε τον δεξιό πίνακα".'</p>").dialog("open");
			return;
		}
		...
		if (relation_stack[rel_txt] != undefined) {
			$("#alert").html("<p>'."Η σχέση με τις επιλεγμένες παραμέτρους υπάρχει ήδη".'</p>").dialog("open");
			return;
		}		
	...
	$("#remrel_btn").click(function(){
		...
		if ($("#rel_list").children(":selected").length > 0) {
			...
			$("#alert").html("<p>'."Επιλέξτε τη σχέση που θέλετε να διαγράψετε".'</p>").dialog("open");
			return;
		}
	});
