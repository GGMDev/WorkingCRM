...
$(document).ready(function(){
	...
	$(".del").click(function(){
		...
		$("#alert")
			...
			.dialog("option", "buttons", {
				"'."Όχι".'": function() { $(this).dialog("close"); },
				"'."Διαγραφή".'": function() {
					$.ajax({
						type: "POST",
						...
						error: function() {
							$("#alert").html("<p>'."Δοκιμάστε αργότερα".'</p>").dialog("open");
						}
					});				
				}
			})
	...
	if(!count($arr_UserGroups))
	{
		$b_includes .= '$("#admin_sql").click(function(){
			$("#alert").dialog("option","title","'."Εισαγωγή κωδικού".'");
			$("#alert")
				.html("'."Κωδικός".':&nbsp;<input type=password id=admin_password size=30 value=\"\">")
				.dialog("option", "buttons", {
					"'."Ακύρωση".'": function() { $(this).dialog("close"); },
					"'."Εντάξει".'": function() {
						$.ajax({
							type: "POST",
							...
							success: function(msg){
								if ( msg == "OK" ) {
									...
									$("#alert").dialog("option", "buttons", {"'."Ακύρωση".'": function() { $(this).dialog("close");}});
									$("#alert").html("<p>'."Όχι αποδεκτός κωδικός".'</p>").dialog("open");
								}
							}
						});				
					}
...
$arr_tables_custom = DBGetTablesListByGroup("custom");
	foreach ( $arrSharedReports as $rpt ) {
		...
		// if @BUILDER.bDynamicPermissions
			if ( $rpt['view'] ) {
				$shared_reports .= '<a class="action view" type="report" href="#">['."Προβολή".']</a>';
			}
			if ( $rpt['edit'] ) {
				if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
					$shared_reports .= '<a class="action edit" type="report" href="#">['."Επεξεργασία".']</a>';
				$shared_reports .= '<a class="action del" type="report" href="#">['."Διαγραφή".']</a>';
			}
			$shared_reports .= '</span>';
			$shared_reports .= '</div>'."\r\n";
		}
	...
	foreach ( $arrSharedCharts as $chart ) {
		...
		// if @BUILDER.bDynamicPermissions
			if ( $chart['view'] ) {
				$shared_charts .= '<a class="action view" type="chart" href="#">['."Προβολή".']</a>';
			}
			if ( $chart['edit'] ) {
				if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
					$shared_charts .= '<a class="action edit" type="chart" href="#">['."Επεξεργασία".']</a>';
				$shared_charts .= '<a class="action del" type="chart" href="#">['."Διαγραφή".']</a>';
			}
			$shared_charts .= '</span>';		
			$shared_charts .= '</div>'."\r\n";
		}
...
foreach ( $arrPrivateReports as $rpt ) {
	if ( $rpt["status"] == "public" ) {
		...
		$private_reports .= '<span class="ritem" id="' . $rpt['name'] . '" title="'.runner_htmlspecialchars($rpt['title']).'">';
				$private_reports .= '<img src="images/unlock16.png" title="'."Κοινόχρηστη έκθεση".'"/>';
		$private_reports .= ( strlen( $rpt['title'] ) > $cMaxTitleLength+5 ) ? substr( $rpt['title'], 0, $cMaxTitleLength  ) . "..." : $rpt['title'];
		$private_reports .= '<a class="action view" type="report" href="#">['."Προβολή".']</a>';
		if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
			$private_reports .= '<a class="action edit" type="report" href="#">['."Επεξεργασία".']</a>';
		$private_reports .= '<a class="action del" type="report" href="#">['."Διαγραφή".']</a>';
		$private_reports .= '</span>';
		$private_reports .= '</div>'."\r\n";
	} else {
		$private_reports .= '<div style="margin-bottom:5px;">';
		$private_reports .= '<span class="ritem" id="' . $rpt['name'] . '" title="'.runner_htmlspecialchars($rpt['title']).'">';
				$private_reports .= '<img src="images/lock16.png" title="'."Προσωπική αναφορά".'"/>';
		$private_reports .= ( strlen( $rpt['title'] ) > $cMaxTitleLength+5 ) ? substr( $rpt['title'], 0, $cMaxTitleLength  ) . "..." : $rpt['title'];
		$private_reports .= '<a class="action view" type="report" href="#">['."Προβολή".']</a>';
		if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
			$private_reports .= '<a class="action edit" type="report" href="#">['."Επεξεργασία".']</a>';
...
foreach ( $arrPrivateCharts as $chart ) {
	if ( $chart["status"] == "public" ) {
		...
		$private_charts .= '<span class="ritem" id="' . $chart['name'] . '" title="'.runner_htmlspecialchars($chart['title']).'">';
				$private_charts .= '<img src="images/unlock16.png" title="'."Κοινόχρηστο γράφημα".'"/>';
		$private_charts .= ( strlen( $chart['title'] ) > $cMaxTitleLength+5 ) ? substr( $chart['title'], 0, $cMaxTitleLength  ) . "..." : $chart['title'];
		$private_charts .= '<a class="action view" type="chart" href="#">['."Προβολή".']</a>';
		if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
			$private_charts .= '<a class="action edit" type="chart" href="#">['."Επεξεργασία".']</a>';
		$private_charts .= '<a class="action del" type="chart" href="#">['."Διαγραφή".']</a>';
		$private_charts .= '</span>';		
		$private_charts .= '</div>'."\r\n";
	} else {
		$private_charts .= '<div style="margin-bottom:5px;">';
		$private_charts .= '<span class="ritem" id="' . $chart['name'] . '" title="'.runner_htmlspecialchars($chart['title']).'">';
				$private_charts .= '<img src="images/lock16.png" title="'."Προσωπικό γράφημα".'"/>';
		$private_charts .= ( strlen( $chart['title'] ) > $cMaxTitleLength+5 ) ? substr( $chart['title'], 0, $cMaxTitleLength  ) . "..." : $chart['title'];
		$private_charts .= '<a class="action view" type="chart" href="#">['."Προβολή".']</a>';
		if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
			$private_charts .= '<a class="action edit" type="chart" href="#">['."Επεξεργασία".']</a>';
...
{
	$create_butt="<a id=\"admin_page\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"admin_page\" href=\"#\">"."Περιοχή Διαχειριστή"."</a>";
	$xt->assign("admin_page", $create_butt);
}
if(!$wr_is_standalone)
{
	$create_butt="<a id=\"return_app\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"return_app\" href=\"#\">"."Επιστροφή στη βασική εφαρμογή"."</a>";
	$xt->assign("back_to_app", $create_butt);
}
if($wr_is_standalone && isWRAdmin())	
{
	$create_butt="<a id=\"users_list\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"users_list\" href=\"#\">"."Λίστα Χρηστών"."</a>";
...
{
	$strLogin="Είσοδος ως"." <b>".$_SESSION["UserName"]."</b>&nbsp;&nbsp;&nbsp;<A class=tablelinks href=\"".GetTableLink("login")."?a=logout\">"."Έξοδος"."</A>";
}
...
if(count($arr_tables_db) || count($arr_tables_project) || count($arr_tables_custom))	
{                 
	$create_butt="<a id=\"report_createbtn\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"report_createbtn\" wtype=\"report\" href=\"#\">"."Δημιουργία Αναφοράς"."</a>";
    $create_butt.="<a id=\"chart_createbtn\" class=\"rnr-button\" onmouseover=\"this.focus();\" name=\"chart_createbtn\" wtype=\"chart\" href=\"#\">"."Δημιουργία διαγράμματος"."</a>";
	$xt->assign("create_report_chart", $create_butt);
}
else
{	
	if($wr_is_standalone && !isWRAdmin())
		$xt->assign("create_report_chart", "<b>"."Δεν έχετε τα δικαιώματα να δημιουργήσετε αναφορές και διαγράμματα. Επικοινωνήστε με τον διαχειριστή. "."</b>");
}
